...
def _build_explanation(
    ...
    if label == "suspicious":
        if applied_rules and model_label != "suspicious" and model_label != "fake":
            rules_str = ", ".join(applied_rules)
            return f"Flagged by rules despite low model score (probability={fake_prob:.2f}): {rules_str}"
